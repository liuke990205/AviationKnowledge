...
class Serializer():
    def __init__(self, never_split: List = None, do_lower_case=True,
                 do_chinese_split=False):  # never_split: tokens 列表，在分词时永远不会被切分。  do_lower_case忽略大小写
    ...
    def _clean_text(self, text):
        ...
        for char in text:
            ...
            if cp == 0 or cp == 0xfffd or self.is_control(
                    char):  # 131行 #从其他字符集向Unicode编码转换时，如果这个二进制数在该字符集中没有标识任何的字符，则得到的结果是0xfffd。
                continue
